...
export const controller = (
  ...
  return {
    ...
    defaultValue: null,
    // deserialize receives the value from the GraphQL query (which is already a JSON object
    // thanks to the backend's output resolver) and passes it to the Field component.
    deserialize: item => {
      return item[config.path] || null;
    },
    // serialize receives the value from the Field component and prepares it for the
    // GraphQL mutation. The backend's input resolver expects a JSON object.
    serialize: value => {
